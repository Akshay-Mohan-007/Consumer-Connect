({
    ...
    saveTab : function(component,isSubmit) {
        ...
        if(sCurrentTab === "CC_TargetingCriteria" && isSubmit){
                var helper = this;
    ...
    submit : function(component,helper){
        ...
        childComp.submit(function(lstNotVisitedTabs){
            if(!$A.util.isEmpty(lstNotVisitedTabs)){//checks if any page to redirect to before submission
                ...
                //Start Akshay[09-Jan-18]: Fix for TKT-000878 - Changed if condition to utilize some method since lstTabs is array of objects
                if(lstTabs.some(function(wrapTab){
                    return lstNotVisitedTabs[0] === wrapTab.name;
                }))
                //End Akshay[09-Jan-18]: Fix for TKT-000878 - Changed if condition to utilize some method since lstTabs is array of objects
