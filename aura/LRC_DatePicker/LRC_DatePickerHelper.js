({
    doInit:function(component,event){
        ...
        //alert("Existing date is"+existingDate);
        if(existingDate){
    ...
    checkTimeValidity:function(component,event){
        ...
        //console.log(hour+':'+minutes+':'+mer+':'+tz);
        
        if(hour != '' && minutes != '' && mer != '' && tz != ''){
            component.set('v.isTimeValid',false);
        }
        else{
           component.set('v.isTimeValid',true); 
        }
   ...
   */
    ...
    selectDate: function(component, event) {
        try {
            ...
            var selectedDate = new Date(component.get("v.year"), component.get("v.month"), component.get("v.date"));
            //var dateStr = component.get("v.year") + "-" + (component.get("v.month") + 1) + "-" + component.get("v.date");
            var selMonth = (component.get("v.month")+1)>10? (component.get("v.month")+1):'0'+(component.get("v.month")+1);
             var selDate = (component.get("v.date"))>9? (component.get("v.date")):'0'+(component.get("v.date"));
            var dateStr = selMonth + "/" + selDate + "/" + component.get("v.year");
            
            //make changes here
            var hour = component.get("v.hour")?component.get("v.hour"):'';
            var minutes = component.get("v.min")?component.get("v.min"):'';
            var mer = component.get("v.meridian")?component.get("v.meridian"):'';
            var tz = component.get("v.timezone")?component.get("v.timezone"):'';
            var newTime;
            if(component.get("v.hour"))
            {
              newTime = hour+':'+minutes+' '+mer+' '+tz;  
            }
            else{
                newTime='';
            }
            ...
            component.set("v.selectedDate", selectedDate);
            component.set("v.value", dateStr+' '+newTime );
            //finally fire the event to tell parent components we have changed the date:
            var dateChangeEvent = component.getEvent("dateChangeEvent");
            dateChangeEvent.setParams({"value" : dateStr });
            dateChangeEvent.fire();
