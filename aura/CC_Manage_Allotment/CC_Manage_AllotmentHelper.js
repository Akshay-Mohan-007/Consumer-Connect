({
    loadPage : function(component,event) {
        ...
        this.callServer(component, "c.loadData", function(response) {
            ...
            this.consoleLog('Page data loaded',false,result);
            
            //component.set("v.sRespWrapper",result);
            //var responseWrapper=component.get("v.sRespWrapper");
            component.set('v.sProgramId',result.WrapperProgDetail.sPid);
    ...
    handleMeetCriteria : function(component,event)  {
        ...
        component.set('v.AppDetObj.MA_Meet_For_Another_Waiver_Program__c', bMeetCriteria);
        //var mameetanother=component.get("v.AppDetObj.MA_Meet_For_Another_Waiver_Program__c");
    ...
    validate : function(component,bIsCreateTask,callback){
        ...
        var bMeetAnotherCriteria=component.get("v.AppDetObj.MA_Meet_For_Another_Waiver_Program__c");
        ...
        if(bIsCreateTask && !bHasError){
            if(!$A.util.isEmpty(appDetObj.Visited_Pages__c) && appDetObj.Visited_Pages__c.includes('Targeting Criteria') 
               && appDetObj.Visited_Pages__c.includes('Urgency of Need')){
                ...
                var lstVist = [];
                //Start Akshay[09-Jan-18]: Fix for TKT-000878 - Changed if condition to check for empty values
                if($A.util.isEmpty(appDetObj.Visited_Pages__c) || !appDetObj.Visited_Pages__c.includes('Targeting Criteria'))
                ...
                bIsCreateTask = false;
                //End Akshay[09-Jan-18]: Fix for TKT-000878 - Changed if condition to check for empty values
            }
        }
        else if(callback)
    ...
    rowClick:function(component,event,helper){
        ...
        if(sAction === 'waitlist'){
            ...
            $A.createComponent("c:CC_WaitListGrid_Component", 
                               {"sProgramName" : component.get('v.ProgramName'),
                                "sProgramDetailId" : wrapRow.sId,
                                "sProgramId" : component.get('v.sProgramId')
                                },
