({
	loadData : function(component) {
		...
		var params = {"sQueueName" : component.get("v.sQueueDevName"), "sSortField" : component.get("v.sSortField"),"sSortOrder" : component.get("v.sSortOrder")};                
        this.callServer(component, "c.loadData", function(response) {
            ...
            //console.log(result);
            var lstOwnerPicklist = result.DashboardInfo.lstOwnerPicklist;
            var mapLabels = result.DashboardInfo.mapLabels;
            var iListSize = result.DashboardInfo.iListSize;
            var lstTotalTasks = result.DashboardInfo.lstMyTasks;
            ...
            component.set("v.lstOwnerPicklist", lstOwnerPicklist);
            component.set("v.sSiteName", sSiteName);
            component.set("v.selectedOwnerId", mapLabels.MY_USER_ID);
            component.set("v.iListSize", iListSize);
            component.set("v.lstTotalTasks", lstTotalTasks);
	...
	getDataList : function(component, gotoLastPage) {
		...
		this.getTaskList(component,
						component.get("v.selectedOwnerId"), 
	...
	getTaskList : function(component, sOwnerId, iOffset, sSortField, sSortOrder, sSearchText, gotoLastPage) {
		//returns the filtered data
		//console.log("getTaskList");
        var params = {"sOwnerId" : sOwnerId,"iOffset" : iOffset,"sSortField" : sSortField,"sSortOrder" : sSortOrder, "sSearchText" : sSearchText, "gotoLastPage" : gotoLastPage};
                        
        this.callServer(component, "c.getMyTaskListAsString", function(response) {
        	//console.log("response>>" + response);
        	if(this.showUnhandledException(component, response))
        		return;
            var result = JSON.parse(response);
            ...
            component.set("v.lstTotalTasks", result.lstTasks);
	...
	updateTable : function(component) {
		...
		var lstTasks = [], iOffset = 0, iOffsetClient = 0;
		...
		var lstTotalTasks = component.get("v.lstTotalTasks");
		...
		if((iOffsetClient + iPageSize) < iListSize){
			lstTasks = lstTotalTasks.slice(iOffsetClient, iOffsetClient + iPageSize);
		} else if(iOffsetClient < iListSize){
        	lstTasks = lstTotalTasks.slice(iOffsetClient, iListSize);
        }
