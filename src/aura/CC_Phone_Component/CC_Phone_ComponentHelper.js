({
    validate : function(component,callback) {
        ...
        if(($A.util.isEmpty(sPhoneType) && !$A.util.isEmpty(sPhoneNo))
            || ($A.util.isEmpty(sPhoneType) && !$A.util.isEmpty(sPhoneConf))
            || ($A.util.isEmpty(component.get('v.sPhoneType')) && bRequired)){ 
            lstErrors.push(mapLabels.CC_Error_Phone_Type_Missing+' '+sPrefix);
            component.set('v.bTypeError',true);
        } 
        ...
        else if(!$A.util.isEmpty(component.get('v.sPhoneType')) && $A.util.isEmpty(sPhoneNo))
        {
            lstErrors.push(mapLabels.CC_Error_Phone_No_Missing+' '+sPrefix+' '+mapLabels.CC_Label_Phone_no);
            component.set('v.bPhoneError',true);
        }
        ...
        var bIsPhoneConfirmEmpty = false;
        if($A.util.isEmpty(sPhoneConf) && bRequired && bConfirm){
            lstErrors.push(mapLabels.CC_Error_Phone_Confirm_Missing+' '+sPrefix);
            bIsPhoneConfirmEmpty = true;
        }
        
        if(bConfirm && !bIsPhoneConfirmEmpty && !$A.util.isEmpty(sPhoneNo) && !$A.util.isEmpty(sPhoneConf) && (sPhoneNo !== sPhoneConf) ){
            lstErrors.push(mapLabels.CC_Error_Phone_Not_Matching+' '+sPrefix);
            component.set('v.bConfirmError',true);
        }
        else if(bConfirm && !bIsPhoneConfirmEmpty && !$A.util.isEmpty(sPhoneNo) && (sPhoneNo !== sPhoneConf)){
            lstErrors.push(mapLabels.CC_Error_Phone_Not_Matching+' '+sPrefix);
            component.set('v.bConfirmError',true);
        }
