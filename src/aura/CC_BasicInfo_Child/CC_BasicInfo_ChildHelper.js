({
	...
	validate : function(component, event, callback) {
		...
		if( !$A.util.isEmpty(dtDOB) ){
			if(!this.isValidDate(this.formatDOB(dtDOB))){
				lstErrors.push(msgError.CC_Err_DOB_Format);
			...
			//this.consoleLog('Format---->');
			//this.consoleLog(this.isValidDate(this.formatDOB(dtDOB)));
			}
